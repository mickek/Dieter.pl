...
def meals_section_view(context, meals, name):
    return { 'MEDIA_URL':context['MEDIA_URL'], 'meals': meals, 'name':name}

def day_plan(context, diet, day):
    """
    Renders day plan.
    Day may be a date object or a dayplan object
    If day is a date object we try to get dayplan based on diet start date
    """
    day_plan = None
    ...
    return { 'MEDIA_URL':context['MEDIA_URL'], 'day_plan': day_plan }
...
register.inclusion_tag('diet/meals_section_view.html',takes_context=True)(meals_section_view)